...
## API Routes
- GET /me - gets information about the currently signed in user (checks the jwt)
- POST /me - updates information on the currently signed in user (things like name updates, etc)
- GET /orders - gets all the most recent orders for the signed in user
- POST /orders - create a new `order` for the signed in user
	- GET /orders/:orderID - gets all info about the specific order
	- POST /orders/:orderID - updates info about the specific order (add/remove products, and let the server sort out which ones are carried by which reps)
		- POST /orders/:orderID/send - sends the order! Fires off all the requisite things that need to happen to get the right emails/texts to the right people.
...
- `bars` - places where liquor is delivered to, managed by multiple bar managers - basically have different views for different bars
...
so it looks like we're gonna need a table to associate products with distributors in zip codes.
